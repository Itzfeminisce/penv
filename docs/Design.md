...
The workflow creates the release once, in a job the six build jobs need, so nothing races to create it and the builds only upload. That job refuses the tag first when `v<tag>` is not the `[workspace.package]` version in `Cargo.toml`, and it is where the two installers are uploaded.
